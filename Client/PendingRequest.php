...
namespace QuantaForge\Http\Client;
...
use QuantaForge\Http\Client\Events\ConnectionFailed;
...
class PendingRequest
{
     ...
     *
     * @var \QuantaForge\Http\Client\Factory|null
     */
    protected $factory;
     ...
     *
     * @var \QuantaForge\Support\Collection|null
     */
     ...
     *
     * @var \QuantaForge\Http\Client\Request|null
     */
    protected $request;
     ...
     *
     * @param  \QuantaForge\Http\Client\Factory|null  $factory
     ...
     *
     * @param  callable  $callback
     * @return array<array-key, \QuantaForge\Http\Client\Response>
     */
    ...
    protected function sendRequest(string $method, string $url, array $options = [])
    {
        ...
        $quantaforgeData = $this->parseRequestData($method, $url, $options);
        ...
        return $this->buildClient()->$clientMethod($method, $url, $this->mergeOptions([
            'quantaforge_data' => $quantaforgeData,
            'on_stats' => $onStats,
        ], $options));
    }
    ...
    protected function parseRequestData($method, $url, array $options)
    {
        ...
        $quantaforgeData = $options[$this->bodyFormat] ?? $options['query'] ?? [];

        $urlString = Str::of($url);

        if (empty($quantaforgeData) && $method === 'GET' && $urlString->contains('?')) {
            $quantaforgeData = (string) $urlString->after('?');
        }

        if (is_string($quantaforgeData)) {
            parse_str($quantaforgeData, $parsedData);

            $quantaforgeData = is_array($parsedData) ? $parsedData : [];
        }

        if ($quantaforgeData instanceof JsonSerializable) {
            $quantaforgeData = $quantaforgeData->jsonSerialize();
        }

        return is_array($quantaforgeData) ? $quantaforgeData : [];
    }
            ...
            return function ($request, $options) use ($handler) {
                ...
                return $promise->then(function ($response) use ($request, $options) {
                    $this->factory?->recordRequestResponsePair(
                        (new Request($request))->withData($options['quantaforge_data']),
        ...
        return function ($handler) {
            return function ($request, $options) use ($handler) {
                $response = ($this->stubCallbacks ?? collect())
                     ->map
                     ->__invoke((new Request($request))->withData($options['quantaforge_data']), $options)
                     ->filter()
                     ->first();
        ...
        return tap($request, function (&$request) use ($options) {
            $this->beforeSendingCallbacks->each(function ($callback) use (&$request, $options) {
                $callbackResult = call_user_func(
                    $callback, (new Request($request))->withData($options['quantaforge_data']), $options, $this
                );
